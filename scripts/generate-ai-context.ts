...
import { ensureRepositoryRoot, listDirectories, pathExists, readJson, walkFiles } from "./_lib/files";
import { extractNavigationUrls, scanAppRoutes } from "./_lib/routes";
...
export async function buildAiContext(repositoryRoot: string): Promise<string> {
  ...
  const dashboardRoot = path.join(appRoot, "(main)", "dashboard");
...
- React Server Components: \`${String(snapshot.shadcn.rsc)}\`
