...
import {
  {{CAMEL_SINGULAR}}Schema,
  type {{PASCAL_SINGULAR}}Input,
} from "../_schemas/{{ENTITY_SINGULAR}}";
...
export function {{PASCAL_SINGULAR}}Form({
  defaultValues,
  onSubmit,
}: {{PASCAL_SINGULAR}}FormProps) {
    ...
    <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-6" noValidate>
      <div className="space-y-2">
        ...
        <Input
          ...
          aria-describedby={
            form.formState.errors.name ? "{{ENTITY_SINGULAR}}-name-error" : undefined
          }
          {...form.register("name")}
        />
